...
| Field | Value |
|---|---|
| Current version | **0.6.9** (HTTP/1.1 + HTTPS; GET/POST/methods + headers + `-A` UA; `-i`/`-I`/`-f`; **`-r` recursive** w/ tree-mirroring + robots.txt `Allow`/`Disallow` precedence, `-O`, `-C` **resume**, `--retry` — over the taar transport; **HTTP + HTTPS both run on AGNOS**, QEMU-validated at 0.6.2, rebuilt at 0.6.3 onto the sovereign `tls_native_*` path) |
...
| AGNOS validation | **QEMU-revalidated at 0.6.9** (virtio-net + SLIRP, KVM): `whirl http://example.com` and `whirl https://example.com` both return the **complete** Example Domain page over the sovereign stack, cert-verified. Six releases of agnos-affecting change had accumulated behind compile-only checking before this run; it found a real defect immediately (see CHANGELOG 0.6.9). **Iron on archaemenid remains open.** |
...
| Area | Defect |
|---|---|
| agnos paths the smoke misses | the CA-hook cache, the `-C` resume guard and `_path_is_symlink`'s agnos branch are still unexecuted — the harness drives only `--help`, an HTTP fetch and an HTTPS fetch (roadmap **A2**) |
